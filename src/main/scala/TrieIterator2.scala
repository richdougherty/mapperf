 ...
 */
private[collection] abstract class TrieIterator2[+T](elems: Array[Iterable[T]]) extends AbstractIterator[T] {
  outer =>
  ...
  } with TrieIterator2[T](xs) {
    final override def getElem(x: AnyRef): T = outer.getElem(x)
  }

  def dupIterator: TrieIterator2[T] = new DupIterator(elems)

  private[this] def newIterator(xs: Array[Iterable[T]]) = new TrieIterator2(xs) {
    final override def getElem(x: AnyRef): T = outer.getElem(x)
  }
  ...
  @tailrec private[this] def next0(elems: Array[Iterable[T]], i: Int): T = {
    if (i+1 >= elems.length || elems(i+1) == null) { // reached end of level, pop stack
      depth -= 1
      if (depth >= 0) {
